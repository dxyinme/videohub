...
| 能力 | 说明 |
|------|------|
| 视频浏览 | 启动时读取配置中的视频根目录，递归或按一层扫描其中的 `.mp4` 文件，在页面上以列表展示 |
...
1. **列表区**：展示视频名称（及可选大小）；点击一项进入播放。
2. **播放区**：`<video controls>`，`src` 指向 `/api/stream/{id...}`。
3. **上传区**：选择本地 `.mp4` 并提交；成功后刷新列表并可直接播放。
4. **空态 / 错误态**：目录无视频或接口失败时给出清晰提示。

### 6.2 交互

- 进入站点默认拉取 `/api/videos`
- 选择视频后更新播放器 `src` 并 `play()`（注意浏览器自动播放策略，可仅设置 src 由用户点播放）
- URL 可用 query（如 `?v=movies%2Ffoo.mp4`）便于刷新保持当前片
...
- [ ] 配置视频目录后，列表能看到其中所有 `.mp4`
- [ ] 点击可在页面内播放，可拖动进度条
- [ ] 可通过页面/API 上传 `.mp4`，上传后出现在列表中
